...
new const FILECONFIGNAME[] = "/darkmod/darkmod.cfg"
new const FILEMAPSNAME[] = "/darkmod/darkmod_maps.cfg"
...
public plugin_cfg()
{
	...
	else
	{
		...
		write_file(filename,"wup_autostart 0")
		write_file(filename,";")
		write_file(filename,";")
		...
		write_file(filename,"wup_mode 0")
		write_file(filename,";")
		write_file(filename,";")
		...
		write_file(filename,";/////////WARMUP PRO FILE\\\\\\\\\\;")
		write_file(filename,";/////////WARMUP PRO FILE\\\\\\\\\\;")
		write_file(filename,"echo WarmUP 5.6 Pro bekzodoff.")
		write_file(filename,"echo WarmUP 5.6 Pro by bekzodoff")
		write_file(filename,";/////////WARMUP PRO FILE\\\\\\\\\\;")
		write_file(filename,";/////////WARMUP PRO FILE\\\\\\\\\\;")
...
{	
	...
	ColorChat(0, RED, "^x03[DARKMOD] %s ^x01WarmUP OFF!", name)
	ExecuteForward(wup_fwd_admindsaible, fwd_resultado, plr);
	server_cmd( "sv_restartround 1;server" );
...
public admin_exec2(plr, level)
{
	...
	server_cmd("exec %s/warmup/warmup.cfg",cfgdir);
	g_can_respawn = true; 
	g_cuchi = true; 
	g_items = true; 
	wup_on = true;
	...
	ColorChat(0, RED, "^x03[DARKMOD] %s ^x01WarmUP ON!", name)
...
{ 
	...
	{  
		...
		if( get_pcvar_num(mostrarhud) == 1 )
		{
			...
			ShowSyncHudMsg(0, sync, "%L", LANG_PLAYER, "DR_WARMUP", time_s);
		}
...
public darresultado()
{
	...
	if( resultadofinal >= 1 )
	{
		if( !g_muchos )
		{
			ColorChat(0, RED, "^x03[DARKMOD] ^x01Winner is ^x03%s ^x01%d ^x01kills", name, resultadofinal)
			
			if( get_pcvar_num(logtofilecvar) == 1 )
				log_to_file(LOGFILE, "^x03[DARKMOD] ^x01Winner is ^x03%s ^x01%d ^x01kills", name, resultadofinal)
			
		}
		else if( g_muchos )
		{
			ColorChat(0, RED, "^x03[DARKMOD] ^x01Winner is ^x03%s ^x01%d ^x01kills", mensajes, resultadofinal)
			
			if( get_pcvar_num(logtofilecvar) == 1 )
				log_to_file(LOGFILE, "^x03[DARKMOD] ^x01Winner is ^x03%s ^x01%d ^x01kills", mensajes, resultadofinal)
	...
	if( resultadofinal == 0 )
	{
		ColorChat(0, RED, "^x03[DARKMOD] ^x01No Winner.")
		
		if( get_pcvar_num(logtofilecvar) == 1 )
			log_to_file(LOGFILE, "^x03[DARKMOD] ^x01No Winner.")
